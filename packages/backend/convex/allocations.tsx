...
export const getAllForCurrentUser = query({
  handler: async (ctx) => {
    ...
    // Get all allocations for these spaces and count occupied parks
    const allocationsWithOccupancy = await Promise.all(
          ...
          allocations.map(async (allocation) => {
            const activeParkingLogs = await ctx.db
              ...
              .filter((q) =>
                q.and(
                  q.eq(q.field('orgId'), orgId),
                  q.eq(q.field('allocationId'), allocation._id),
                  q.eq(q.field('status'), 'active')
                )
              )
              .collect();

            const occupiedParks = activeParkingLogs.length;
    ...
    // Flatten the array of allocations with occupancy
    return allocationsWithOccupancy.flat();
  },
});
      ...
      const driverSpaces = await ctx.db
        ...
        .filter((q) =>
          q.and(
            q.eq(q.field('orgId'), orgId),
            q.or(
              ...drivers.map((driver) => q.eq(q.field('userId'), driver.userId))
            )
          )
        )
        .collect();
