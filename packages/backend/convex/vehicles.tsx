...
export const fetchCarJamDetails = action({
  args: { plate: v.string() },
  handler: async (ctx, args) => {
    const API_KEY = '	681810D6137325945BECCE49E30C50E16E7DF123';
    const BASE_URL = 'https://www.carjam.co.nz/a/vehicle:abcd';

    const carJamUrl = `${BASE_URL}?key=${API_KEY}&plate=${args.plate}`;
    ...
    try {
      ...
      console.error('Error fetching CarJam details:', error);
      return {
        success: false,
...
export const getAllForUser = query({
  handler: async (ctx) => {
    ...
    const activeParkingLogs = await ctx.db
      .query('parkingLogs')
      .filter((q) =>
        q.and(q.eq(q.field('orgId'), orgId), q.eq(q.field('status'), 'active'))
      )
      .collect();

    // Create a Set of parked vehicle IDs for quick lookup
    const parkedVehicleIds = new Set(
      activeParkingLogs.map((log) => log.vehicleId)
    );
    ...
    const userVehicles = allVehicles
      .filter((vehicle) => {
        if (vehicle.availableTo === 'space') {
          ...
          return vehicle.drivers.some(
            (driver: { id: string }) => driver.id === occupant._id
          );
        }
        return false;
      })
  ...
  handler: async (ctx, args) => {
    ...
    if (args.isDropoff !== undefined) {
      filteredVehicles = vehicles.filter((vehicle) =>
        args.isDropoff
          ? !activeVehicleIds.has(vehicle._id)
          : activeVehicleIds.has(vehicle._id)
      );
    }
