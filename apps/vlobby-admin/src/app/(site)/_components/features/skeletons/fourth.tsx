...
export const SkeletonFour = () => {
  return (
    <div className="p-8 overflow-hidden h-full relative flex flex-col group">
      ...
      <Container className="mt-10 ml-4">Announcements</Container>
      <Container className="mt-4 ml-10 group-hover:border-secondary transition duration-200 group-hover:scale-[1.02]">
        Social Feed
      </Container>
      <Container className="mt-4 ml-4">Clubs & Events</Container>
      <Cursor
...
const Cursor = ({
  ...
  return (
    ...
    >
      ...
      <div
        className={cn(
          'absolute top-3 left-3 p-1 rounded-md  text-[10px] whitespace-pre text-foreground  transition duration-200',
          textClassName
        )}
      >
        {text ?? 'Manu Arora'}
...
const Container = ({
  ...
  return (
    <div
      className={cn('p-0.5 rounded-lg border border-muted w-fit', className)}
    >
      <div
        className={cn(
